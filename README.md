...
| Skill | 说明 |
|-------|------|
| [`cdp-chrome`](skills/cdp-chrome/SKILL.md) | 共享有头 Chrome 实例管理。所有需要 GUI 浏览器的场景（社交媒体、登录态网站、反 bot 页面）必须遵循此 Skill。含启动脚本和 MCP 配置模板。 |
| [`telegram-agents`](skills/telegram-agents/SKILL.md) | Telegram agent 配置与管理。包括 tmux 会话、Telethon 调度器、launchd 心跳定时任务。 |
...
| 命令 | 说明 |
|------|------|
| [`/check-release`](commands/check-release.md) | 检查 Claude Code 版本更新，通过 Telegram 发送发布报告或 Anthropic 新闻简报。 |
| [`/song <query>`](commands/song.md) | 搜索歌词、翻译为中文，收集趣闻和流行文化梗。 |
| [`/task-init <name>`](commands/task-init.md) | 创建任务目录并编写需求文档。 |
| [`/task-run <name>`](commands/task-run.md) | 启动开发-评估反馈循环。 |
| [`/tg-status`](commands/tg-status.md) | 查看所有 Telegram agent 的运行状态。 |
| [`/tg-restart <agent>`](commands/tg-restart.md) | 重启指定 Telegram agent 的 channel 会话。 |
| [`/tg-logs [lines]`](commands/tg-logs.md) | 查看心跳调度器日志。 |
...
| Server | 说明 |
|--------|------|
| [`telegram-notify`](mcp-servers/telegram-notify/) | 轻量级 Telegram 通知服务，供 agent 发送消息。 |
