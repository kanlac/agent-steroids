...
- **订阅 YAML 顶层下发 `ipv6: false`**：避免客户端解析 AAAA 走 IPv6 直连绕过代理（IPv6 路由泄露——地理严格的站如 Gemini 会因此判错国家、报 not supported）。注意 WebRTC 读网卡 IPv6 的泄露订阅管不了，需设备/路由器关 IPv6，详见 [[clash-verge-config]]。
