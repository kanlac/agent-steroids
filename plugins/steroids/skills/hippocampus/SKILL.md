...
- **预置记忆（每轮预载）**——每次对话都被载入上下文：全局 `~/.claude/CLAUDE.md`、项目 `CLAUDE.md` / `AGENTS.md`、原生 auto-memory。它是最贵的地段，因为**每一轮**都在付租金。超过预算后 harness 会静默丢弃尾部，于是一个吃太饱的预置层不只是浪费 token——它停止加载、悄悄什么都不再指引，还把周围一切变得更吵。预置多不等于知道得多，而是遵循得更少。**预置层必须稀缺。**
...
1. **诊断**——扫描记忆环境（全局记忆、项目记忆、可触达的 skill 及 description、指令文件指向的文档；只扫**此时此地这个** Agent，不跑去扫别的项目）。自动探测存在哪些记忆文件（`CLAUDE.md` / `AGENTS.md` / auto-memory——有啥扫啥，不假设一定是 Claude）。四维打分（体量 / 可用性 / 新鲜度 / 矛盾），每扣一分都**挂证据**（`file:line` + 原文引用），生成诊断报告。
